...
class Pix2GestaltOccluPaths:
    """统一管理文件命名与子目录；若你的文件夹名不同，可以在初始化 Dataset 时传入覆盖。"""
    root: str
...
class Pix2GestaltOccluDataset(Dataset):
    """
    将 RGB / 可见掩码 / 伪深度 拼为 5 通道输入，并读取 SAM feats 与非模态 whole mask。
    - 输入5通道顺序: [R, G, B, visible_mask, pseudo_depth]
    - SAM feats: .pt 字典中的 'feats' -> 期望形状 (1, 256, 64, 64)
    - 返回字典字段: key, x5, sam_feats, y_whole, paths
    """
    ...
    def _scan_samples(self, keys_subset: Optional[List[str]]) -> List[str]:
        ...
        pbar = tqdm(total=len(keys), desc="扫描样本", unit="file", disable=not use_tqdm) if use_tqdm else None
        ...
        if len(valid_keys) == 0 and len(missing_all) > 0 and self.strict_check:
            examples = [f"key={k} 缺失: {','.join(miss)}" for k, miss in missing_all[:20]]
            more = "" if len(missing_all) <= 20 else f"... 以及另外 {len(missing_all) - 20} 个样本"
            raise FileNotFoundError("数据集中没有有效样本。\n" + "\n".join(examples) + ("\n" + more if more else ""))

        if self.strict_check and len(missing_all) > 0 and len(valid_keys) > 0:
            print(missing_all)
            print(f"[WARN] 缺失文件的样本已跳过，示例: {missing_all[0][0]} -> {missing_all[0][1]} (共 {len(missing_all)} 个)")
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # 拼 5 通道 (R,G,B,visible,depth) -> CHW
